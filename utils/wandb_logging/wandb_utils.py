...
def process_wandb_config_ddp_mode(opt):
    ...
    if isinstance(data_dict['train'], str) and data_dict['train'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        train_path = Path(train_dir) / 'data/train/'
    ...
    if isinstance(data_dict['val'], str) and data_dict['val'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        val_path = Path(val_dir) / 'data/train/'
    ...
    def setup_training(self, opt, data_dict):
        ...
        if 'val_artifact' not in self.__dict__:  # If --upload_dataset is set, use the existing artifact, don't download
            ...
            if self.train_artifact_path is not None:
                train_path = Path(self.train_artifact_path) / 'data/train/'
                data_dict['train'] = str(train_path)
            if self.val_artifact_path is not None:
                val_path = Path(self.val_artifact_path) / 'data/train/'
                data_dict['val'] = str(val_path)
    ...
    def create_dataset_table(self, dataset, class_to_id, name='dataset'):
        ...
        for img_file in img_files:
            if Path(img_file).is_dir():
                artifact.add_dir(img_file, name='data/train')
                labels_path = 'train'.join(dataset.path.rsplit('train', 1))
                artifact.add_dir(labels_path, name='data/train')
            else:
                artifact.add_file(img_file, name='data/train/' + Path(img_file).name)
                label_file = Path(img2label_paths([img_file])[0])
                artifact.add_file(str(label_file),
                                  name='data/train/' + label_file.name) if label_file.exists() else None
